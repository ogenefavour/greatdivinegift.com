...
class Student{
    ...
    public function resultClasscount($conn,$sid,$sessid,$termid){
        ...
        $classcount=$row['thecount'];
        if(!empty($classcount)){
            if($classcount < 20){ //manage display for classes less than 20
                $classcount=" ";
            }
            return $classcount;
        }else{
    ...
    public function getSubjectCountr($conn,$sid,$classid,$sessid,$termid){
        $sqlcount="SELECT COUNT(DISTINCT subject_id) AS thecount FROM result WHERE class='$classid'AND student_id ='$sid' AND session_id='$sessid' AND term_id ='$termid''";
        $result=  mysqli_query($conn,$sqlcount);
        $row=mysqli_fetch_assoc($result);
  ...
  //get total number of JSS1 students
    ...
    public function getTotalStudent22($conn,$schoolid){
        ...
        if(!empty($totalstud)){
           // $totalstud=" "; // by chisom making total student display nothing
            return $totalstud;
        }
    }
