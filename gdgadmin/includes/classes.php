...
class Admin{
    ...
    public function totalAverage($conn,$classid,$sessid,$termid,$totalscore){
       // modified by chisom if($classid<=3){
            $count = 11;
       ...
       // }
        ...
        $sqlcount22="SELECT COUNT(DISTINCT subject_id) AS thecount FROM result WHERE class='$classid' AND session_id='$sessid' AND term_id ='$termid'";
    ...
    public function getTotalStudent13($conn,$schoolid){
        ...
        if(!empty($totalstud)){
          ...
          //get total number of JSS3c students
    public function getTotalStudent13($conn,$schoolid){
        $sql = "SELECT COUNT(student_id) AS counts FROM students WHERE schoolid = '$schoolid' AND class = 23";
        $res = mysqli_query($conn,$sql);
        while($row=mysqli_fetch_assoc($res)){
            $totalstud = $row['counts'];
        }
