...
class Admin{
    ...
    public function resultClasscount($conn,$sid,$sessid,$termid){
        ...
        $classcount=$row['thecount'];
        if(!empty($classcount)){
            if($classcount < 20){ // mange display of classes less than 20 in result sheet
                $classcount=" ";
            }
            return $classcount;
        }else{
    ...
    public function getTotalMark($conn,$sid,$classid,$sessid,$termid){
        $sqlsum="SELECT SUM(total_score) AS totalsum FROM result WHERE class='$classid' AND student_id ='$sid' AND session_id='2' AND term_id ='1'";
    ...
    //get total number of courses a class offers
    public function getScores($conn,$subjectid,$studentid,$classid,$sessid,$termid){    //editted by Chisom addede parameters -$classid,$sessid,$termid and used numbers instead of variable for temporary use as this isnt fetching -->   AND session_id='$sessid' AND term_id ='$termid'
        $sqlsubsco = "SELECT  subject_id,total_score FROM result WHERE student_id = '$studentid' AND subject_id = '$subjectid' AND class= '$classid' AND term_id='$termid' AND session_id='$sessid' ";
    ...
    public function totalAverage($conn,$totalscore,$classid,$sessid,$termid){
         /*   if($classid=1){
            $count = 10;
        } elseif($classid==11){
            $count =11;
     ...
     */   
        ...
        if($totalaverage < 40){
            ...
            $remark = 'An average performance but you can always do better if you dedixcate more time to your studies';
            ...
            $remark = 'A very good performance  but do not relax, there are lots more ground to cover';
        }elseif($totalaverage >= 80 && $totalaverage <=89 ){
            $remark = 'An excellent performance. The world is at your feet and we are so proud of you.';
  ...
  //get total number of JSS1 students
    ...
    public function getTotalStudent22($conn,$schoolid){
        ...
        if(!empty($totalstud)){
           // $totalstud = " "; // making to total student return nothing
            return $totalstud;
        }
    }
